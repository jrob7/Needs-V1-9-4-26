// Screens/BusinessOffersModal.js
// Business profile → Offers → this modal for redemption and history.
...
const OFFER_COLORS = {
  orange:   { bg: '#C2710C', text: '#fff' },
  blue:     { bg: '#1D4ED8', text: '#fff' },
  offwhite: { bg: '#F5F0E8', text: '#1F2937' },
};
...
// ── Confirm Redemption popup ──────────────────────────────────────────────────
function ConfirmPopup({ visible, code, offer, onConfirm, onCancel, loading }) {
  const oc = OFFER_COLORS[offer?.bgColor] || OFFER_COLORS.orange;
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onCancel}>
      <View style={styles.confirmOverlay}>
        <View style={styles.confirmBox}>
          <View style={[styles.confirmBadge, { backgroundColor: oc.bg }]}>
            <Ionicons name="checkmark-circle" size={28} color={oc.text} />
          </View>
          ...
          {!!offer?.headline && <Text style={styles.confirmOffer}>{offer.headline}</Text>}
          {!!offer?.needCoins && (
            <View style={styles.confirmCoins}>
              <Ionicons name="star" size={14} color="#F59E0B" />
              <Text style={styles.confirmCoinsText}> {offer.needCoins} NeedCoins will be deducted from the customer</Text>
            </View>
          )}
          <View style={styles.confirmBtns}>
            ...
            <TouchableOpacity
              style={[styles.confirmBtn, { backgroundColor: oc.bg }]}
              onPress={onConfirm}
              disabled={loading}
            >
              {loading
                ? <ActivityIndicator color="#fff" size="small" />
                : <Text style={[styles.confirmBtnText, { color: oc.text }]}>Confirm</Text>
              }
            </TouchableOpacity>
          </View>
        </View>
...
// ── Main Modal ────────────────────────────────────────────────────────────────
export default function BusinessOffersModal({ visible, onClose, restaurantDoc }) {
  ...
  const offer = restaurantDoc?.offer;
  ...
  const loadHistory = async () => {
    ...
    setHistLoading(true);
    try {
      const res = await fetch(`${NODE_API}/offerCodes/history?restaurantId=${restaurantId}`);
  ...
  const onBarcodeScanned = ({ data }) => {
    ...
    // Accept codes in XXXX-XXXX format
    if (/^[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(raw)) {
      setCode(raw);
      setConfirmVisible(true);
    } else {
      Alert.alert('Invalid QR', 'This QR code does not match an offer code format.', [
  ...
  const handleConfirm = async () => {
    ...
    try {
      const clean = codeInput.trim().toUpperCase();
      const res = await fetch(`${NODE_API}/offerCodes/redeem`, {
        method: 'POST',
      ...
      if (res.status === 402) {
        Alert.alert(
          'Not Enough NeedCoins',
          data.message || "Sorry, this customer doesn't have enough NeedCoins yet. Encourage them to keep earning rewards!",
          [{ text: 'OK' }]
        );
        setConfirmVisible(false);
        return;
      ...
      if (!res.ok) {
        Alert.alert('Redemption failed', data.error || 'Could not redeem this code.');
        setConfirmVisible(false);
        return;
      }
      setConfirmVisible(false);
      ...
      Alert.alert(
        '✅ Redeemed!',
        `${data.needCoins} NeedCoins collected from customer.\n"${data.offerHeadline}"`,
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={onClose}>
      <View style={styles.container}>
        ...
        {/* Offer card */}
        {offer && (
          <View style={[styles.offerCard, { backgroundColor: oc.bg }]}>
            <View style={[styles.offerStatusDot, { backgroundColor: offer.enabled ? '#4ADE80' : '#9CA3AF' }]} />
            <Text style={[styles.offerCardTitle, { color: oc.text }]}>{offer.headline || 'Unnamed offer'}</Text>
            {!!offer.description && (
              <Text style={[styles.offerCardDesc, { color: oc.text }]}>{offer.description}</Text>
            )}
            <View style={styles.offerCardMeta}>
              {!!(offer.timeStart && offer.timeEnd) && (
                <Text style={[styles.offerCardMetaTxt, { color: oc.text }]}>
                  {offer.timeStart} – {offer.timeEnd}
                </Text>
              )}
              {!!offer.needCoins && (
                <Text style={[styles.offerCardMetaTxt, { color: oc.text }]}>
                  {'  ·  '}{offer.needCoins} NeedCoins
                </Text>
              )}
            </View>
            <View style={[styles.offerBadge, { backgroundColor: offer.enabled ? '#15803D' : '#6B7280' }]}>
              <Text style={styles.offerBadgeText}>{offer.enabled ? 'Active' : 'Inactive'}</Text>
            </View>
          </View>
        )}
              ...
              {scanning ? (
                ...
                <>
                  <Text style={styles.redeemInstruction}>
                    Enter the customer's code manually or scan their QR code.
                  </Text>
                  ...
                  <View style={styles.inputRow}>
                    ...
                    />
                    <TouchableOpacity
                      style={[styles.submitCodeBtn, { backgroundColor: oc.bg }]}
                      onPress={handleSubmitCode}
                    >
                      <Text style={[styles.submitCodeText, { color: oc.text }]}>Redeem Code</Text>
                    </TouchableOpacity>
                  </View>
            ...
            {histLoading ? (
              ...
              history.map((r, i) => (
                <View key={i} style={styles.histRow}>
                  <View style={[styles.histIconWrap, { backgroundColor: oc.bg + '22' }]}>
                    <Ionicons name="checkmark-circle" size={18} color={oc.bg} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.histCode}>{r.code}</Text>
                    <Text style={styles.histOffer} numberOfLines={1}>{r.offerHeadline || 'Offer redeemed'}</Text>
                    <Text style={styles.histDate}>{fmtDate(r.redeemedAt)}</Text>
                  </View>
                  <View style={[styles.histCoins, { backgroundColor: '#FFFBEB' }]}>
...
const styles = StyleSheet.create({
  ...
  offerCardTitle: { fontSize: 18, fontWeight: '900', marginBottom: 4, paddingRight: 20 },
  offerCardDesc:  { fontSize: 13, opacity: 0.85, marginBottom: 8 },
  offerCardMeta:  { flexDirection: 'row', marginBottom: 10 },
  offerCardMetaTxt: { fontSize: 12, opacity: 0.8 },
  offerBadge: {
  ...
  submitCodeText: { fontSize: 14, fontWeight: '700' },
  ...
  histIconWrap: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  histCode:  { fontSize: 15, fontWeight: '800', letterSpacing: 2, color: '#111', marginBottom: 2 },
  histOffer: { fontSize: 12, color: '#6B7280', marginBottom: 2 },
  histDate:  { fontSize: 11, color: '#9CA3AF' },
  histCoins: { borderRadius: 10, paddingHorizontal: 8, paddingVertical: 5, flexDirection: 'row', alignItems: 'center', gap: 3 },
  ...
  confirmBox: {
    ...
    padding: 28, width: '84%', alignItems: 'center',
  },
  confirmBadge: {
  ...
  confirmOffer: { fontSize: 15, fontWeight: '700', color: '#111', marginBottom: 8 },
  confirmCoins: {
